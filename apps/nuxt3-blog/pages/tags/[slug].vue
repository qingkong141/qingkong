...
const { data: postsData, pending } = await useAsyncData(
  `tag-posts-${slug}-${currentPage.value}`,
  () => usePostApi().list({ page: currentPage.value, pageSize, status: 'published', tagId: tag.value!.id }),
  { watch: [currentPage] },
)
...
<template>
  <div class="container">
    <header class="page-header">
      <NuxtLink to="/tags" class="back-link">← 所有标签</NuxtLink>
      <h1 class="page-title">
        <span
          v-if="tag!.color"
          class="tag-dot"
